...
import { paddleConfigured } from '@/lib/billing';
...
export async function GET() {
  ...
  if (!account)
    return Response.json({ needsOnboarding: true, databaseEngine: 'MySQL' });
  ...
  for (const row of validSources) {
    ...
    if (!current || price < current.price)
      bestByProduct.set(productId, { price, merchant: String(row.merchant) });
  }
  return Response.json({
    clients,
    products: products.map((row) => ({
      ...row,
      best: bestByProduct.get(Number(row.id)) ?? null,
    })),
    sources,
    account: {
      ...
      trialDaysRemaining: Number(account.trialDaysRemaining),
      databaseEngine: 'MySQL',
      dataRegion: 'EU',
    ...
    metrics: {
      ...
      watchedUrls: sources.length,
      validPrices: validSources.length,
      issues: sources.filter((row) => row.error || row.isPriceAnomaly === 1)
        .length,
    },
  });
}
